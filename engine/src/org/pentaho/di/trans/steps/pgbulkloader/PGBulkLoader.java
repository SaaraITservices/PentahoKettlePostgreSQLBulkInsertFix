  ...
  public boolean processRow( StepMetaInterface smi, StepDataInterface sdi ) throws KettleException {
    ...
    try {
      ...
      if ( r == null ) { // no more input to be expected...
        ...
        if ( data != null ) {
            // If source have no values pgCopyOut.flush() will fail because object was not initiated. Fixed 2016.03.15 Burenin Alexey dexalex84@gmail.com
        	if (pgCopyOut!=null) {
        	  	pgCopyOut.flush();
          		pgCopyOut.endCopy();
          }
          else 
        	  logBasic( "0 rows bulk loaded! Possible empty source.");
        ...
        if ( valueData != null ) {
          switch ( valueMeta.getType() ) {
            ...
            case ValueMetaInterface.TYPE_BINARY:
                if ( valueMeta.isStorageBinaryString() ) {
                  pgCopyOut.write( (byte[]) valueData );
                }else //valueMeta.getBinaryString(valueData)
                {
                	Byte_str =  "\\x"+Hex.encodeHexString( (byte[]) valueData );
                	pgCopyOut.write( Byte_str.getBytes() );
                }
